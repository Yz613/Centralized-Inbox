...
export async function syncMailbox(db: D1Database, inbox: any) {
  ...
  if (!lease.meta.changes) return { success: true, busy: true, pending: true };
  try {
    ...
    await saveMailThreads(db, result.threads, db.prepare(`UPDATE mailbox_sync SET cursor_json = ?, last_success_at = ?, error = NULL, pending = ?, lease_until = 0 WHERE inbox_id = ?`)
    ...
    return { success: true, pending: result.pending, fetched: result.fetched, folder: result.folder };
  } catch (error: any) {
